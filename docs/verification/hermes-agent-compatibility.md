...
| 2026-06-05 20:03-20:08 KST | `nousresearch/hermes-agent:latest` `sha256:b6e41c155d6bfce5ad83c5d0fec670086db8a43250e4511c9474134be5482d33` | `9501 (65ef50a0a)` | `qwen3.5-2b-q4-xl` / `qwen3.5-2b-ud-q4-k-xl` | OK with host-port caveat | Fresh hostuid data directory test: previous `.hermes-local-llm-hostuid/` was moved to `.hermes-local-llm-hostuid.backup.20260605_200350/`, then `init-hostuid`, `up-hostuid`, and `smoke-hostuid` were run. Startup seeded `config.yaml` and `SOUL.md`; smoke returned `hermes gateway local llm ready` with usage `prompt_tokens=14402`, `completion_tokens=50`, `total_tokens=14452`. Container-internal API/dashboard were healthy, while WSL-side curl to published host ports failed with connection refused. |

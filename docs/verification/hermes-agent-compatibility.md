...
| 2026-06-05 20:03-20:14 KST | `nousresearch/hermes-agent:latest` `sha256:b6e41c155d6bfce5ad83c5d0fec670086db8a43250e4511c9474134be5482d33` | `9501 (65ef50a0a)` | `qwen3.5-2b-q4-xl` / `qwen3.5-2b-ud-q4-k-xl` | OK | Fresh hostuid data directory test: previous `.hermes-local-llm-hostuid/` was moved to `.hermes-local-llm-hostuid.backup.20260605_200350/`, then `init-hostuid`, `up-hostuid`, and `smoke-hostuid` were run. Startup seeded `config.yaml` and `SOUL.md`; smoke returned `hermes gateway local llm ready` with usage `prompt_tokens=14402`, `completion_tokens=50`, `total_tokens=14452`. Container-internal API/dashboard were healthy. Unrestricted host checks also returned 200 for `127.0.0.1:48642/v1/health` and `127.0.0.1:49119/chat`; an earlier non-escalated Codex sandbox curl failure was not a runtime failure. Follow-up smoke returned usage `prompt_tokens=14402`, `completion_tokens=48`, `total_tokens=14450`. |
| 2026-06-05 20:39-20:41 KST | `nousresearch/hermes-agent:latest` `sha256:b6e41c155d6bfce5ad83c5d0fec670086db8a43250e4511c9474134be5482d33` | `9501 (65ef50a0a)` | `qwen3.5-2b-q4-xl` / `qwen3.5-2b-ud-q4-k-xl` | OK | Hostuid startup now creates `/opt/data/.env`, creates the `~/.local/bin/hermes` symlink, and appends missing local provider defaults. `hermes doctor` now reports `/opt/data/.env file exists`, `API key or custom endpoint configured`, and `~/.local/bin/hermes -> correct target`; the only remaining issue is optional external API keys for full tool access. `smoke-hostuid` returned `hermes gateway local llm ready`; usage was `prompt_tokens=14608`, `completion_tokens=33`, `total_tokens=14641`. |
| 2026-06-05 20:57-20:59 KST | `nousresearch/hermes-agent:latest` `sha256:b6e41c155d6bfce5ad83c5d0fec670086db8a43250e4511c9474134be5482d33` | `9501 (65ef50a0a)` | `qwen3.5-2b-q4-xl` / `qwen3.5-2b-ud-q4-k-xl` | OK | Fixed browser `/chat` for hostuid mode by mounting `HERMES_TUI_DIST_DIR=./.hermes-local-llm-hostuid-ui-tui-dist` to `/opt/hermes/ui-tui/dist`. Before the fix, `/chat?resume=20260605_111558_c4f41d` showed `Chat unavailable: 1` and container logs showed `TUI build failed` with `permission denied` writing `entry.js`. After recreating the container, a direct `/api/pty` websocket check returned `101 Switching Protocols` and terminal bytes instead of `Chat unavailable`; `/opt/hermes/ui-tui/dist/entry.js` was created as uid/gid 1000. `smoke-hostuid` still returned `hermes gateway local llm ready`; usage was `prompt_tokens=14608`, `completion_tokens=31`, `total_tokens=14639`. |
...
| 2026-06-05 20:55 KST | `nousresearch/hermes-agent:latest` | `qwen3.5-2b-q4-xl` | Browser chat failed while API smoke passed | Hostuid mode ran as uid/gid 1000 and served dashboard HTML, but `/api/pty` failed to spawn the embedded TUI. Browser showed `Chat unavailable: 1`; logs showed `TUI build failed` because esbuild could not write `/opt/hermes/ui-tui/dist/entry.js` inside the image filesystem. Fixed by adding a writable host bind for `/opt/hermes/ui-tui/dist`. |
...
- Tool payloads are preserved by the gateway. If a llama.cpp version ignores
  ...
  it in the gateway.
- For the full Hermes runtime, prefer `scripts/smoke_hermes_runtime.sh` for
  end-to-end provider checks. The hostuid runtime's published API and dashboard
  ports were also verified at `127.0.0.1:48642` and `127.0.0.1:49119`.

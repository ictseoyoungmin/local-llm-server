...
| 2026-06-05 20:03-20:14 KST | Windows 11 + WSL2 Ubuntu 22.04 + Docker Desktop | fresh `./.hermes-local-llm-hostuid` on `/mnt/f`; previous directory moved to `.hermes-local-llm-hostuid.backup.20260605_200350/` | DrvFS / Windows drive bind mount | hostuid direct-entrypoint mode from fresh init | `nousresearch/hermes-agent:latest` `sha256:b6e41c155d6bfce5ad83c5d0fec670086db8a43250e4511c9474134be5482d33` | OK | `init-hostuid` then `up-hostuid` created a fresh data directory and startup logs showed `Seeded /opt/data/config.yaml` and `Seeded /opt/data/SOUL.md`. `smoke-hostuid` returned `hermes gateway local llm ready`; usage `prompt_tokens=14402`, `completion_tokens=50`, `total_tokens=14452`. Container-internal API/dashboard checks returned 200. Unrestricted host checks also returned 200 for `http://127.0.0.1:48642/v1/health` and `http://127.0.0.1:49119/chat`; an earlier non-escalated Codex sandbox curl failure was not a runtime failure. A follow-up `smoke-hostuid` returned usage `prompt_tokens=14402`, `completion_tokens=48`, `total_tokens=14450`. |
| 2026-06-05 20:57-20:59 KST | Windows 11 + WSL2 Ubuntu 22.04 + Docker Desktop | `./.hermes-local-llm-hostuid` plus `./.hermes-local-llm-hostuid-ui-tui-dist` on `/mnt/f` | DrvFS / Windows drive bind mount | host bind -> `/opt/data`; host bind -> `/opt/hermes/ui-tui/dist`; compose `user: 1000:1000` | `nousresearch/hermes-agent:latest` `sha256:b6e41c155d6bfce5ad83c5d0fec670086db8a43250e4511c9474134be5482d33` | OK | Added a separate host-writable TUI dist bind mount for browser `/chat`. This stores only the dashboard embedded TUI build output, not Hermes DB state. Direct websocket verification against `/api/pty?resume=20260605_111558_c4f41d` returned `101 Switching Protocols`; `/opt/hermes/ui-tui/dist/entry.js` was created as uid/gid 1000. `smoke-hostuid` returned `hermes gateway local llm ready`; usage `prompt_tokens=14608`, `completion_tokens=31`, `total_tokens=14639`. |

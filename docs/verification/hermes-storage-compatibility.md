...
| 2026-06-05 20:03-20:08 KST | Windows 11 + WSL2 Ubuntu 22.04 + Docker Desktop | fresh `./.hermes-local-llm-hostuid` on `/mnt/f`; previous directory moved to `.hermes-local-llm-hostuid.backup.20260605_200350/` | DrvFS / Windows drive bind mount | hostuid direct-entrypoint mode from fresh init | `nousresearch/hermes-agent:latest` `sha256:b6e41c155d6bfce5ad83c5d0fec670086db8a43250e4511c9474134be5482d33` | OK with host-port caveat | `init-hostuid` then `up-hostuid` created a fresh data directory and startup logs showed `Seeded /opt/data/config.yaml` and `Seeded /opt/data/SOUL.md`. `smoke-hostuid` returned `hermes gateway local llm ready`; usage `prompt_tokens=14402`, `completion_tokens=50`, `total_tokens=14452`. Container-internal API and dashboard checks returned 200, but WSL-side curl to Docker-published `127.0.0.1:48642/49119` failed with connection refused. |
